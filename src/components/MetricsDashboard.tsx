...
import { BarChart3, CheckCircle, Clock, Activity } from 'lucide-react'
import {
  ResponsiveContainer,
  BarChart,
  Bar,
...
export default function MetricsDashboard() {
  ...
  const fetchMetrics = async () => {
    try {
      const res = await fetch('/api/metrics?token=marcus2026', { credentials: 'include' })
      if (res.ok) {
    ...
    <div className="bg-gray-900 rounded-lg p-4 md:p-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 mb-6">
        <div className="bg-gray-800 rounded-lg p-3">
          <h3 className="text-sm font-medium text-gray-300 mb-2">7-Day Task Trend</h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={metrics.dailyTrend} margin={{ top: 8, right: 12, left: -8, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="created" name="Created" stroke="#60a5fa" strokeWidth={2} dot={{ r: 3 }} />
        ...
        <div className="bg-gray-800 rounded-lg p-3">
          <h3 className="text-sm font-medium text-gray-300 mb-2">Tasks by Status</h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={statusChartData} margin={{ top: 8, right: 12, left: -8, bottom: 16 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                <XAxis dataKey="status" stroke="#94a3b8" fontSize={12} angle={-18} textAnchor="end" height={46} />
                <YAxis stroke="#94a3b8" fontSize={12} allowDecimals={false} />
                <Tooltip
                  contentStyle={{ backgroundColor: '#0f172a', border: '1px solid #334155', borderRadius: '8px' }}
                  labelStyle={{ color: '#e2e8f0' }}
                />
                <Bar dataKey="count" name="Tasks" fill="#a78bfa" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>

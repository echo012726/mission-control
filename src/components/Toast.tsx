...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const showToast = useCallback((message: string, type: ToastType = 'info', duration = 4000, action?: { label: string; onClick: () => void }) => {
    const id = Math.random().toString(36).substring(2, 11)
    setToasts((prev) => [...prev, { id, message, type, duration, action }])
    
    // Auto-remove after specified duration (longer if action is present)
    const toastDuration = action ? 10000 : duration
    setTimeout(() => {
            ...
            >
              ...
              {toast.action && (
                <button
                  ...
                  className="text-white font-semibold text-sm hover:underline px-2 py-1 rounded bg-white/20 hover:bg-white/30 transition-colors"
                >
                  {toast.action.label}
                </button>
              )}

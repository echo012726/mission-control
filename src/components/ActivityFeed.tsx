...
export default function ActivityFeed() {
  ...
  const fetchEchoActivities = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/&openclaw/activity?limit=20', { credentials: 'include' })
      if (res.ok) {
        const data = await res.json()
  ...
  const getActivityDescription = (activity: Activity) => {
    ...
    try {
      ...
      if (activity.type === 'agent_error') {
        return { title: label, subtitle: `${AGENT_EMOJIS[payload.agentId] || '🤖'} ${payload.agentId}: ${payload.error}` }
      }
    } catch {
      // Payload not JSON
    }
